...
public class StartPage {
    ...
    public void btnEvent() {
        loginBTN.addActionListener(inBtnPressed);
        registerBTN.addActionListener(inBtnPressed);
    }

    private ActionListener inBtnPressed = e -> {
        if (e.getSource() == loginBTN) {

            login = new Login();
            validUser = login.success();

        } else if (e.getSource() == registerBTN) {

            register = new Register();
            validUser = register.success();
        }
        if (validUser) { // clean
            frame.dispose();
        }
